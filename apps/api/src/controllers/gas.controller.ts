...
export async function updateGasReport(req: Request, res: Response) {
  try {
    const id = String(req.params.id);
    const validation = createGasReportSchema.partial().safeParse(req.body);
    if (!validation.success) {
      res.status(400).json({ success: false, error: validation.error.errors[0].message });
      return;
    }
    const payload = validation.data;
    const report = await prisma.gasReport.update({ where: { id }, data: payload });
    res.json({ success: true, data: report });
  } catch (e) {
    console.error(e);
    res.status(500).json({ success: false, error: 'Error interno del servidor' });
